...
SELECT COUNT(ROLL_id) no_of_rolls from customer_orders

--How many unique customer orders were made?

select count(distinct customer_id) no_of_customers from customer_orders
...
select roll_id,count(roll_id) rolls_ordered
from cancel_details c
join customer_orders cd on c.order_id = cd.order_id
where order_cancel_details = 'nc'
group by roll_id

-- How many Veg rolls and non veg rolls were ordered by each customer?

select customer_id, c.roll_id,count(c.roll_id) cnt,roll_name
